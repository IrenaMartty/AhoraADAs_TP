...
const renderCategoryOptions = (categories) => {
  ...
  ).innerHTML += `<option value="Todas">Todas</option>`;
...
const editCategory = (categoryId) => {
  const categoryEdited = getData("categories").find(
...
const deleteCategory = (categoryId) => {
  const updatedCategories = getData("categories").filter(
    (category) => category.id != categoryId
  );
  setData("categories", updatedCategories);
...
const getOperationById = (operationId) =>
  getOperations().find(({ id }) => id === operationId);
...
const renderOperations = (operations) => {
  ...
  if (operations.length) {
    ...
    showElement([".operations-table-container"]);
    for (const operation of operations) {
      ...
      const formattedAmount =
        operation.type === "ganancia"
          ...
          : `-$${operation.amount}`;
      const amountColor =
        operation.type === "ganancia" ? "text-emerald-500" : "text-rose-500";
      // console.log (categorySelected)
      $(".tbody-info-render").innerHTML += `
        <tr class="">
            <td class="sm:pr-6 text-left">${operation.description}</td>
            <td class="px-3 py-1 text-s text-emerald-600 text-left font-bold max-md:hidden">${
            ...
            <td>
                <button class="containerEditOperation-btn text-sky-500 hover:text-black" data-id onclick="editForm('${
                  operation.id
                }')">Editar</i></button>
                <button type="button" class="btn removeOperation-btn text-sky-500 hover:text-black" data-id onclick="ejecutionDeleteBtn('${
                  operation.id
                }','${operation.description}')">Eliminar</button>
            </td>
        </tr>
        `;
    }
    ...
    hideElement([".operations-table-container"]);
  }
};
...
const editForm = (operationId) => {
  ...
  const operationEdit = getData("operations").find(
...
const applyFilters = () => {
  ...
  if (selectType !== "Todos") {
    filteredData = filteredData.filter(
      (operation) => operation.type === selectType
    );
  }
  ...
  if (selectCategory !== "Todas") {
    filteredData = filteredData.filter(
      (operation) => operation.category === selectCategory
    );
  }
...
const totalbyCategory = () => {
  return operations.reduce((acc, operation) => {
    const { category, type, amount } = operation;
...
const categoryWithHighestIncome = (comparison) => {
  const totalNumbers = totalbyCategory();
  let highestCategory = Object.keys(totalNumbers)[0];

  for (const category in totalNumbers) {
    if (
      totalNumbers[category][comparison] >
      totalNumbers[highestCategory][comparison]
    ) {
      highestCategory = category;
    }
  }
  const categoryName = getCategoryNameById(highestCategory);
  const highestAmount = totalNumbers[highestCategory][comparison];
...
const monthWithHighestIncome = (comparison) => {
  ...
  for (const year in totalNumbers) {
    for (const month in totalNumbers[year]) {
      const currentAmount = totalNumbers[year][month][comparison];
...
const renderCategorySum = (title, categoryType, amount) => {
  const { categoryName, highestAmount } =
    categoryWithHighestIncome(categoryType);
  $(".summary").innerHTML += `
    ...
    <div class="flex justify-between md:w-1/3">
        ...
        <span class="font-medium ${
          amount < 0 ? "text-rose-500" : amount > 0 ? "text-emerald-500" : ""
...
const renderMonthSum = (title, comparison, amount) => {
  const { highestMonth, highestAmount } = monthWithHighestIncome(comparison);
  $(".summary").innerHTML += `
    <li class="my-5 md:flex md:justify-between">
        <p class="mb-2 font-medium md:mb-0">${title}</p>
        <div class="flex justify-between md:w-1/3">
            <span>${getMonthAndYear(
              highestMonth.month,
              highestMonth.year
            )}</span>
            <span class="font-medium ${
              amount < 0 ? "text-rose-500" : "text-emerald-500"
            }">${amount > 0 ? "+" : "-"}$${Math.abs(highestAmount)}</span>
        </div>
    </li>`;
};

const renderHighestIncomeCategory = () =>
  renderCategorySum("Categoría con mayor ganancia", "totalIncome", 1);
const renderHighestCostCategory = () =>
  renderCategorySum("Categoría con mayor gasto", "totalCost", -1);
const renderHighestBalanceCategory = () =>
  renderCategorySum("Categoría con mayor balance", "totalBalance", 0);
const renderHighestIncomeMonth = () =>
  renderMonthSum("Mes con mayor ganancia", "totalIncome", 1);
const renderHighestCostMonth = () =>
  renderMonthSum("Mes con mayor gasto", "totalCost", -1);
...
const renderTotalbyCategory = () => {
  ...
  for (const category in totalAmountsByCategory) {
    const { totalIncome, totalCost, totalBalance } =
      totalAmountsByCategory[category];
    const balanceColor =
      totalBalance > 0 ? "text-emerald-500" : "text-rose-500";

    $(".reports-total-by-category").innerHTML += `
            <tr>
                <td class="pr-4 pb-5 font-medium md:pr-0">${getCategoryNameById(
                  category
                )}</td>
...
const renderTotalbyMonth = () => {
  ...
  for (const year in totalAmountsByMonth) {
    for (const month in totalAmountsByMonth[year]) {
      const { totalIncome, totalCost, totalBalance } =
        totalAmountsByMonth[year][month];
      const balanceColor =
        totalBalance > 0 ? "text-emerald-500" : "text-rose-500";
      $(".reports-total-by-months").innerHTML += `
                <tr>
                    <td class="pr-4 pb-5 font-medium md:pr-0">${getMonthAndYear(
                      parseInt(month),
                      parseInt(year)
                    )}</td>
                    <td class="pr-4 pb-5 text-green-500 text-right md:pr-0">+$${totalIncome}</td>
...
const initializeApp = () => {
  ...
  $("#btn-newOperation").addEventListener("click", () => {
  ...
  const removeOperationButtons = document.querySelectorAll(
    ".removeOperation-btn"
  );
  ...
  $(".btn-confirm-delete-operation").addEventListener("click", () => {
    const operationIdToDelete = $(".btn-confirm-delete-operation").getAttribute(
      "data-operation-id"
    );
  ...
  $("#input-description-text").addEventListener("blur", () =>
    validation("descriptionValidation")
  );
  $("#input-amount").addEventListener("blur", () =>
